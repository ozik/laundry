...
if(isset($_POST['hapus'])){
      $Id_karyawan=$_POST['id_karyawan'];
       $query = mysqli_query($koneksi,"SELECT Foto FROM tb_karyawan WHERE Id_karyawan='$Id_karyawan'");
          $data= mysqli_fetch_array($query);
         $fotolama=$data['Foto'];
     if(mysqli_query($koneksi,"DELETE FROM tb_karyawan WHERE Id_karyawan='$Id_karyawan';")){
         if($fotolama != ''){
             unlink("../assets/img/karyawan/" . $fotolama);
            }
...
if(isset($_POST['simpan'])){
    ...
    //upload foto
      ...
      $foto = $_POST['id_karyawan']."_".date('d-m-Y').".".$extensi;
      $sumber=$_FILES['foto']['tmp_name'];
      $ukuran = $_FILES['foto']['size'];
      if($ukuran <= 2000000){
          if( ($extensi == "jpg") or ($extensi == "png") or ($extensi == "JPG") or ($extensi == "PNG")){
                        if(mysqli_query($koneksi,"INSERT INTO tb_karyawan VALUES ('$Id_karyawan','$nama','$alamat','$jk',$tgl_lahir,'$no_telp','$foto','$username','$password');")){
                            $upload =move_uploaded_file($sumber, "../assets/img/karyawan/" .$foto);
                            echo ('<div class="alert alert-success alert-dismissable">
                                  <button type="button" class="close" data-dismiss="alert" aria-hidden="true">×</button>
...
if(isset($_POST['edit'])){
    ...
    //upload foto
      ...
      $ukuran = $_FILES['foto']['size'];
      if($file == ''){
        if(mysqli_query($koneksi,"UPDATE tb_karyawan SET Nama='$nama',alamat='$alamat',Jk='$jk',Tgl_lahir=$tgl_lahir,No_telp='$no_telp' WHERE Id_karyawan='$Id_karyawan';")){
                            echo ('<div class="alert alert-success alert-dismissable">
                                  <button type="button" class="close" data-dismiss="alert" aria-hidden="true">×</button>
                                  <strong>Data berhasil disimpan</strong>
                                  </div>');
      ...
      if($ukuran <= 2000000){
          if( ($extensi == "jpg") or ($extensi == "png") or ($extensi == "JPG") or ($extensi == "PNG")){
                        if(mysqli_query($koneksi,"UPDATE tb_karyawan SET Nama='$nama',alamat='$alamat',Jk='$jk',Tgl_lahir=$tgl_lahir,No_telp='$no_telp',Foto='$foto' WHERE Id_karyawan='$Id_karyawan';")){
                            $upload =move_uploaded_file($sumber, "../assets/img/karyawan/" .$foto);
                            echo ('<div class="alert alert-success alert-dismissable">
                                  <button type="button" class="close" data-dismiss="alert" aria-hidden="true">×</button>
                                  <strong>Data berhasil disimpan</strong>
...
?>
      <tr>
          ...
          <td align="center">
              <a id="btn_detail" data-toggle="modal" data-target="#form_detail" data-id_karyawan="<?php echo $data["Id_karyawan"];?>" data-nama="<?php echo $data["Nama"];?>" data-alamat="<?php echo $data["Alamat"];?>" data-jk="<?php echo $data["Jk"];?>" data-tgl_lahir="<?php echo $data["Tgl_lahir"];?>" data-no_telp="<?php echo $data["No_telp"];?>" data-foto="<?php echo $data["Foto"];?>" >
                          <button type="button" class="btn btn-info btn-trans btn-xs" title="Detail">
                              <i class="fa fa-eye"></i>
                          </button>
              </a>
              <a id="btn_edit" data-toggle="modal" data-target="#form_edit" data-id_karyawan="<?php echo $data["Id_karyawan"];?>" data-nama="<?php echo $data["Nama"];?>" data-alamat="<?php echo $data["Alamat"];?>" data-jk="<?php echo $data["Jk"];?>" data-tgl_lahir="<?php echo $data["Tgl_lahir"];?>" data-no_telp="<?php echo $data["No_telp"];?>" data-foto="<?php echo $data["Foto"];?>" >
                          <button type="button" class="btn btn-warning  btn-trans btn-xs" title="Edit">
                              <i class="fa fa-pencil"></i> 
                          </button>
               </a>
               <a id="btn_hapus" data-toggle="modal" data-target="#form_hapus" data-id_karyawan="<?php echo $data["Id_karyawan"];?>" data-nama="<?php echo $data["Nama"];?>" >
...
<!--popup tambah--> 
<?php
$query = mysqli_query($koneksi,'SELECT max(Id_karyawan) as maxId FROM tb_karyawan');
$data = mysqli_fetch_array($query);
$id = $data['maxId'];
                ...
                <div class="modal-body modal-scroll">
                    <form class="form-horizontal" role="form" method="post" action="" enctype="multipart/form-data">
                        <div class="form-group">
                            <label class="col-sm-3 control-label" for="Id_karyawan">ID karyawan</label>
                            <div class="col-sm-9">
                                <input type="text" class="form-control" name="id_karyawan" id="Id_karyawan" value="<?php echo($id)?>" title="Id terisi otomatis" readonly>
                            </div>
                        </div>
                        <div class="form-group">
                ...
                <div class="modal-body modal-scroll" id="modal_edit">
                    <form class="form-horizontal" role="form" method="post" action="" enctype="multipart/form-data">
                        <div class="form-group">
                            <label class="col-sm-3 control-label" for="id_karyawan">ID Karyawan</label>
                            <div class="col-sm-9">
                                <input type="text" class="form-control" name="id_karyawan" id="id_karyawan"  title="Id tidak dapat diubah" readonly>
                            </div>
                        </div>
                        <div class="form-group">
                            <label class="col-sm-3 control-label"  for="nama">Nama</label>
...
$(document).on('click', '#btn_detail', function(){
  ...
  $("#modal_detail #id_karyawan").html(id_karyawan);
  ...
  $("#modal_detail #foto").attr("src", "../assets/img/karyawan/"+foto);
  } );
</script>
</div> 
<!--end popup detail--> 
...
$(document).on('click', '#btn_hapus', function(){
  var Id_karyawan = $(this).data('id_karyawan');
  var nama = $(this).data('nama');
  $("#modal_hapus #id_karyawan").val(Id_karyawan);
