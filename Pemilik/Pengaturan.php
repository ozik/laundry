...
if(isset($_POST['btn_password'])){
      ...
      $query = mysqli_query($koneksi,"SELECT Password FROM tb_pemilik WHERE Id_pemilik='$id_pemilik'");
      $data = mysqli_fetch_array($query);
      $dbpass = $data['Password'];
      if ($dbpass==$password_lama) {
        if ($password_baru==$ulangi_password) {
                  if(mysqli_query($koneksi,"UPDATE tb_pemilik SET Password='$password_baru' WHERE Id_pemilik='$id_pemilik';")){
...
if(isset($_POST['btn_username'])){
      ...
      $query = mysqli_query($koneksi,"SELECT Username FROM tb_pemilik WHERE Id_pemilik='$id_pemilik'");
      $data = mysqli_fetch_array($query);
      $dbpass = $data['Username'];
      if ($dbpass==$Username_lama) {
        if ($Username_baru==$ulangi_Username) {
                  if(mysqli_query($koneksi,"UPDATE tb_pemilik SET Username='$Username_baru' WHERE Id_pemilik='$id_pemilik';")){
                            echo ('<div class="alert alert-success alert-dismissable">
                                  <button type="button" class="close" data-dismiss="alert" aria-hidden="true">×</button>
